...
from utils import AudioPlayerWebRTC, AudioRecorderWebRTC, check_audio_devices
...
async def main() -> None:
    ...
    openai_client = AsyncAzureOpenAI(
        ...
        api_key=os.environ["AZURE_OPENAI_API_KEY"],
        azure_deployment=os.environ["AZURE_VOICE_COMPLETION_DEPLOYMENT_NAME"],
        api_version="2024-12-17",
    )

    print("Using OpenAI API key: ", os.environ["AZURE_OPENAI_API_KEY"])
    print("Using OpenAI API endpoint: ", os.environ["AZURE_OPENAI_ENDPOINT"])
    print("Using OpenAI API deployment name: ", os.environ["AZURE_VOICE_COMPLETION_DEPLOYMENT_NAME"])
    print("Using OpenAI API model: ", os.environ["AZURE_VOICE_COMPLETION_MODEL"])

    realtime_agent = OpenAIRealtimeWebRTC(client=openai_client, audio_track=AudioRecorderWebRTC(), plugins=[Helpers()])
